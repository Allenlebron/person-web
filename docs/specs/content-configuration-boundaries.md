...
| 首页布局         | 保存 `layoutPreset` 并输出页面属性              | 当前首页组件固定为 `ShelfHome`，还没有不同布局组件 |
...
- 中英文内容：`getHomeCopy`
- 首页硬编码外链：`https://01mvp.com`、`https://makerjackie.com`
...
- 页脚 `Powered by 01mvp.com` 和 `Created by Jackie`
- 默认导航回退项
