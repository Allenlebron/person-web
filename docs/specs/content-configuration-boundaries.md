...
- 首页固定界面文案、关于页主体介绍、Demo、帮助文档和页面布局，目前由代码管理。
...
| 站点描述         | 首页 Hero、页脚、RSS、部分元信息                | 首页固定区块标题和说明仍由代码管理                   |
...
| 导航链接         | 桌面端公开页头导航                              | 含 Demo/docs 的模板导航会回退为个人博客默认导航      |
| 主语言           | RSS 内容语言、部分默认语言行为                  | 中英文界面文案本身仍在代码消息文件中                 |
| 主题预设         | 公开站、后台、登录页样式                        | 可选项由代码限定                                     |
| 首页布局         | 后台不再展示该字段；底层保留 `layoutPreset` 值  | 当前首页组件固定为 `ContentHome`，还没有不同布局组件 |
...
- Hero 和作者区中的站点名称、描述、作者名称、作者简介
- 页头和页脚中的站点名称、描述、导航、社交链接
- 主题预设
- 首页展示的精选文章、最新文章、专栏和标签内容

代码硬编码：

- Hero 辅助标题和按钮文案
- 精选、最新文章、专栏、标签和关于区块的界面文案
- 首页区块顺序、卡片结构和配色
- 联系邮箱 `myzwilpan@gmail.com`

主要修改入口：

- 页面结构：`HomePage`、`ContentHome` 等组件
- 中英文内容：`getHomeCopy`
...
- 页脚版权年份格式
- 含 Demo/docs 的导航会回退为文章、专栏、标签和关于入口
- 移动端底部导航的首页、文章、专栏、标签和账号入口
...
| `/sitemap.xml`       | 文章、标签、专栏和站点 URL                  | 固定页面列表                    |
...
`apps/web/public/robots.txt` 已移除，生产环境由动态 `/robots.txt` 路由统一输出。Demo 和文档页仍可直接访问，但不会出现在个人博客导航和 sitemap 中。
